class Heap:
  ...
  def delete(self):
    ...
    self.storage.pop()
    # call _sift_down to move the element at index 1 down to a valid spot in
    self._sift_down(1)
    return retval
  ...
  def _bubble_up(self, index):
    ...
    while index // 2 > 0 :
      # check to see if the element's parent's value is less than the current
      ...
      # our heap element has reached a valid spot in the heap
        ...
        break
      # update index value to be the parent's index so that we can contains
      index = index // 2
  def _sift_down(self, index):
    # keep sifting the element at the give index down the heap until it reach
  ...
  def _max_child(self, index):
    if index * 2 + 1 > self.size:
      ...
      return index * 2 if self.storage[index *2] > self.storage[index*@+1] else index * 2 + 1